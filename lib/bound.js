...
Bound.prototype.updateContent = function(contentPath, repository) {
  ...
  return fs.exists(contentPath).then(_.bind(function(exists) {
    ...
    return this.exec(['git', 'fetch']).then(_.bind(function() {
      return this.exec(['git', 'rebase']);
    }, this));
  }, this))['catch'](function(e) {
    console.error('Failed to update content from remote repository.', e ? e.message : '');
  })['finally'](function() {
    process.chdir(cwd);
  });
};
        ...
        return queue.then(function() {
          _.each(contents, function(files, section) {
            ...
            sorted = _.sortBy(files, function(file) {
              return new sortValueType(file.meta[config.sort.key]);
            });
...
exports.update = function() {
  ...
  return bound.writeManifest().then(function(contents) {
    ...
    console.error(new Date().toGMTString(), error ? error.stack || error.message || error.toString() : 'Failed for unknown reason.');
  }).then(function() {
